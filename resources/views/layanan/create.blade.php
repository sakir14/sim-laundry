<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-black text-2xl text-slate-800">Tambah Layanan</h2>
                <p class="text-sm text-slate-500">Buat paket layanan baru</p>
            </div>
        </div>
    </x-slot>
    <div class="py-8">
        <div class="max-w-lg mx-auto px-4 sm:px-6 lg:px-8">
            <div data-aos="fade-up" class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <form action="{{ route('layanan.store') }}" method="POST" class="p-6 space-y-5">
                    @csrf
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1.5">Nama Layanan <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_layanan" value="{{ old('nama_layanan') }}" class="input-modern" placeholder="Contoh: Cuci Setrika Regular" required>
                        <x-input-error :messages="$errors->get('nama_layanan')" class="mt-1" />
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1.5">Harga (Rp) <span class="text-red-500">*</span></label>
                            <input type="number" name="harga" value="{{ old('harga') }}" class="input-modern" placeholder="5000" min="0" required>
                        ...
                        <div>
                            ...
                            <select name="satuan" class="input-modern" required>
                                ...
                                <option value="pcs" {{ old('satuan')=='pcs' ? 'selected' : '' }}>Pcs (Satuan)</option>
