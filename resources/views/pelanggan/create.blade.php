<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-black text-2xl text-slate-800">Tambah Pelanggan</h2>
                <p class="text-sm text-slate-500">Daftarkan pelanggan baru</p>
            </div>
        </div>
    </x-slot>
    <div class="py-8">
        <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
            <div data-aos="fade-up" class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <form action="{{ route('pelanggan.store') }}" method="POST" class="p-6 space-y-5">
                    @csrf
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-1.5">Nama Pelanggan <span class="text-red-500">*</span></label>
                        <input type="text" name="nama_pelanggan" value="{{ old('nama_pelanggan') }}" class="input-modern @error('nama_pelanggan') border-red-400 @enderror" placeholder="Nama lengkap" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="no_hp" value="{{ old('no_hp') }}" class="input-modern @error('no_hp') border-red-400 @enderror" placeholder="Contoh: 08123456789" required>
                    ...
                    <div>
                        ...
                        <textarea name="alamat" rows="3" class="input-modern @error('alamat') border-red-400 @enderror" placeholder="Alamat lengkap" required>{{ old('alamat') }}</textarea>
